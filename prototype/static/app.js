...
function updateAgProg(){
  const done=AGENTS.filter(a=>(BOARD[a.k]||{}).s==='done').length;
  $('#agprog').textContent=done+' / '+AGENTS.length;
}
...
function finishBoard(where){
  ...
  AGENTS.forEach(a=>{
    const b=BOARD[a.k]||{};
    if(agentNode(a)<from){BOARD[a.k]={s:'done',how:'reused',base:base};}
    else if(!boardSawEvent){rerun.push(a.k); return;}
    else if(b.s==='done'&&b.how==='event'){b.pending=false; b.ev=null;}   /* payload 為準 */
    else BOARD[a.k]={s:'done',how:'poll'};
    paintCard(a.k);
  });
  updateAgProg();
  if(boardSawEvent){setAgSource('event',where); boardComplete(where?'幕僚團已依新執行結果更新':'');return;}
  setAgSource('replay',where);
  replayAgents(rerun,'replay',()=>boardComplete(where?'幕僚團已依新執行結果更新':''));
}
