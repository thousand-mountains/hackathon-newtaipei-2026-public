...
**`tool_result.status`（新增，必要）**　值域 `ok | empty | not_attempted | failed`
（**2026-09-13 由三個變成四個**，見下）。

| status | 意思 | 前端該說什麼 |
|---|---|---|
| `ok` | 正常完成 | 顯示結果 |
| `empty` | **做了，結果是沒有**。`hits[]` 為空 | 「這個條件下沒有找到」 |
| `not_attempted` | **沒做，因為前置條件不成立**——這一步根本還沒發生 | 「還沒查」，並照 `note` 講清楚缺什麼、下一步做什麼 |
| `failed` | **工具本身失敗**（KB 打不到、快照載不動、pipeline 炸了） | 「查詢來源失敗，可重試」 |

> 為什麼一定要分 `empty` 與 `failed`：檢索失敗在 `ChatTools._search` 內部就被攔下轉成文字回給模型，
> **不會冒到 `error` 事件**。沒有 `status`，前端看到的兩者長得一模一樣，會把「查詢失敗」
> 畫成「資料庫裡沒有這筆資料」。

> **為什麼 2026-09-13 補上 `not_attempted`。** 在那之前 `empty` 同時承載兩件事：
>
> - 「查過了，沒有這筆資料」——真的查無
...
> 一條路，而那種比對下次改字就悄悄失效。
>
> **系統說「沒有」跟系統說「我還沒去看」是兩種不同的誠實等級**（CONSTITUTION §1）。
> 把後者講成前者，是在報告一件沒發生的事。
>
> **判準：做了、結果是沒有 → `empty`；沒做、因為前置條件不成立 → `not_attempted`。**
> 「做了但結果不完整」算 `empty`——工作確實發生了。

目前四個值實際由誰發（後端 `backend/llm/chat.py`，由 AST 檢查釘住值域）：

| 情境 | status |
|---|---|
| 檢索真的跑了、零筆命中 | `empty` |
| `read_case` 讀到的分區是空的（讀了，裡面沒東西） | `empty` |
| 關聯圖畫出來了，但這個 run 只跑到程序審查、沒有結論句 | `empty`（圖有帶出去） |
| 工具 chip 導不出查詢詞，**檢索層一次都沒被呼叫** | `not_attempted` |
| 還沒有任何 run，關聯圖**一次都沒去畫** | `not_attempted` |
...
**這帶來一個要顯示的狀態，而且是三態不是兩態**（2026-09-13 更正）。

使用者挑的法規是當**查詢詞**餵回 N4，而法條查表只認「法名第N條」——manifest 的 `t` 是
純法規名（法規檔整部一檔），**所以對通道 A 幾乎必然零命中，但對通道 B（相似案語意檢索）
確實生效**（實測：`search("廢棄物清理法")` → 0 命中、`search("廢棄物清理法第2條")` → 1 命中；
而 `n4_query` 確實出現在 `retrieval_meta.case_query_extra_terms`）。

| `retrieval_status` | 意思 | 右欄標示 |
|---|---|---|
| `matched` | 進了法條查表，草稿可引用 | 已進入法條查表，草稿可引用本法條 |
...
> `matched` **不代表「因為使用者挑了才查到」**。通道 A 是 N4 依案情獨立檢索
> （2026-09-05 拍板），常見法規挑不挑都會被查到。系統回答得了「它在不在結果裡」，
> **回答不了「它是不是因你而在」**——文案因此寫「已進入法條查表」，不寫「你的挑選生效了」。

**欄位**：`tool_result.picked_laws`（原 `unmatched_laws`）。回**每一筆**挑選
`{id, t, state, note}`，**不是只回沒中的那幾筆**——只回沒中的話，畫面分不出
「這條進了查表」與「這條我根本沒算」。

這一條與 §3.7 的 `flagged`／`unlinked` 是同一件事的兩面。
