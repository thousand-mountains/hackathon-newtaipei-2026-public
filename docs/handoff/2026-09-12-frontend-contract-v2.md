...
#### 3.5.2 ⚠️ 手動加進右欄的法規／案例，目前不會進到草稿

**這一條跟前端現有文案矛盾，要先拍板。**

前端現在的擋門文案寫著「草稿**只認右側卷宗裡的東西**當來源」（`store/app.js:310`）。
**那句話跟後端不符。** N5 取的是 `state.retrieval`（N4 自己檢索的結果，`n5_draft.py:121-122`），
**不是** manifest 的本案清單。使用者手動搜尋加入的五條法規，N5 一條都看不到。

後果：承辦人挑了法規、按生成草稿、草稿一條都沒引 → 會以為系統壞了。

**為什麼是這樣**：2026-09-05 Ci 拍板改成 N4 獨立檢索，理由寫在 `graph.py:488`——
舊版把「草稿即將引用的法條」餵給 N4，讓「引用一定查得到」變成必然，
**檢索佐證的是自己**。這個決定是對的，不要直接推翻。

**兩條路，要 Ci 拍板：**

| | 做法 | 代價 |
|---|---|---|
| **(a)** | **維持現況**：手動清單只是承辦人的參考資料，不影響草稿 | 零工。但**前端文案必須改**——不能再說「只認右側卷宗」。右欄手動加入的項目要標「參考資料，不影響草稿生成」 |
| **(b)** | 把使用者挑的法條**當查詢詞**餵進 `overrides.n4_query`（`graph.py:489-496` 的既有例外口） | 小工。**不違反 2026-09-05 那個拍板**——`n4_query` 是查詢詞不是答案，N4 仍然要自己查得到才會進 `laws[]`，循環佐證的問題不會回來 |

**建議 (b)**，因為 (a) 會讓右欄的「搜尋並加入法規」對辦案流程幾乎沒有作用——
使用者做了一件事，系統沒有任何反應，那個功能就只是裝飾。
...
| **「草稿只認右側卷宗裡的東西當來源」** | `store/app.js:310` | **文案與後端不符**（§3.5.2）。N5 取的是 N4 自己檢索的結果，不是右欄的手動清單。依 §3.5.2 拍板結果改文案或改實作 |
