...
## 四點五、⚠ 這份程式碼**沒有**通過獨立的第三方審查

依「產出的人不驗自己」的規矩，我在完工後派了一個 fresh-context 的 opus agent 做對抗式
審查（它沒看過我的產製過程，只看最終結果）。**它跑了一個多小時沒有回報，我催過一次
也沒回應，最後我沒有等到它的結論就收尾了。**

所以請你把這份程式碼當成「**只有作者自己驗過**」來看待：

- ✅ 有的：59 條自動化測試、18 組手動對抗輸入、實際啟動服務打過每一支端點、
  實際 build 並跑過容器。這些是**可重跑的客觀證據**，不是「我覺得沒問題」。
- ❌ 沒有的：獨立第三方對抗式審查。上面所有測試都是我自己寫的，
  **我可能在我沒想到的地方留了洞，而我寫的測試不會去測我沒想到的東西**。
- 具體風險：Bug 1 與 Bug 2 都是我自己找到的，這證明我會找 bug，但也證明
  **這類 bug 確實會發生**。第三個、第四個很可能還在裡面。

**建議**：merge 之前派一個 fresh agent 跑一次 `/code-review`，或你自己看一遍
`backend/gate/citations.py` 與 `backend/nodes/n6_gate.py` 這兩個檔——
守門是整個系統的紅線所在，其他檔壞了頂多難看，這兩個檔壞了會讓系統放行假引用。
...
3. 跑一次 `python3 backend/tests/run_all.py` 自己確認（59/59、exit 0），再決定要不要 merge。
...
```
403252b fix(gate): 全形數字造成合法引用被誤攔（誤判為查無此號並阻擋送出）
