...
export const real = {
  ...
  getLaw: (lawId) => http.get(`/laws/${enc(lawId)}`), // #11 看全文
  ...
  removeCaseLaw: (id, lawId) => http.del(`/cases/${enc(id)}/laws/${enc(lawId)}`), // #14

  // ── §1.4 相似案例／參考決定（母庫唯讀 + 本案清單 C/R/D）──
  searchDecisions: (q) => http.get(`/decisions?q=${enc(q)}`), // #15 母庫查
  getDecision: (decisionId) => http.get(`/decisions/${enc(decisionId)}`), // #16 看全文
  ...
  removeReference: (id, refId) => http.del(`/cases/${enc(id)}/references/${enc(refId)}`), // #19
