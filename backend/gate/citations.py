...
_DATE = r"[0-9０-９]{2,3}\s*年\s*[0-9０-９]{1,2}\s*月\s*[0-9０-９]{1,2}\s*日"
_WORD = r"[一-龥]{2,8}字"
...
DIRECTIVE_PATTERNS = (
    ...
    re.compile(rf"({_AGENCY})?\s*(?:{_DATE}\s*)?({_WORD})\s*第\s*([0-9０-９]+)\s*號\s*{_SUFFIX}"),
    # 2) 無後綴，但有完整發文日期（年月日）——判解只有「年度」，不會有月日，故不會誤收
    re.compile(rf"({_AGENCY})?\s*{_DATE}\s*({_WORD})\s*第\s*([0-9０-９]+)\s*號"),
    # 3) 無後綴、無日期，但機關名緊鄰字別（「內政部台內營字第…號」）
    re.compile(rf"({_AGENCY})\s*({_WORD})\s*第\s*([0-9０-９]+)\s*號"),
)
