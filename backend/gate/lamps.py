...
# 模型標的 cite_ids 有一部分不在白名單內（N4 的 laws/cases ∪ 當次工具命中），
# 已由 `llm/client.py` 清空。清空是對的，但**不能只是清空就算了**：那句話原本是
# 靠那個引用在指出處的，引用消失之後它就是一句沒有出處的話，必須外顯成紅燈。
# 文案要指名被清掉的是什麼（{dropped}），否則承辦人看不出模型想引什麼。
WHY_UNSUPPORTED_CITATION = (
    "模型引用了檢索結果之外的來源（{dropped}），已清除，該句視為無出處；"
    "後端送出端點會以 409 拒絕本案送出。"
)
