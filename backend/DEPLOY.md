...
- **環境收回時間未經一手確認，而且兩種說法差了兩天。** 這關係到「交出去的網址會不會在評審點開之前就死掉」，
  不是文件細節。

  | 說法 | 來源 | 性質 |
  |---|---|---|
  | 環境開放至 **2026-09-13 13:00**，屆時 AWS 開發環境關閉 | 主辦方投影片（決賽開幕說明會） | **一手、書面** |
  | 帳號 **2026-09-15 08:00** 收回 | 無一手來源；回推自 Workshop Studio 事件頁的 Duration 72 小時 | **推論** |

  主辦方（Bruce）口頭提過「環境開 72 小時，但以活動時間為準，明天 13:00 後即使環境還開著也交不了」——
  這句話**針對的是交件，不是環境存活**，而且「即使還開著」反而暗示 13:00 後不必然關閉。
  也就是說：**兩邊都沒有把環境實際關閉的時刻講死。**

  在向賽方一手確認之前，本文件與相關產物**不主張 9/15**。規劃時以 **9/13 13:00 可能即失效**為準備基準，
  較保守的那個假設出錯的代價比較小。賽後若需長期可存取，需另備帳號——超出本 change 範圍，需 Ci 拍板。
