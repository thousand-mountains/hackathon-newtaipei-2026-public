...
from backend.config.settings import OUTPUT_DIR, run_mode
...
def _print_summary(payload: dict[str, Any]) -> None:
    ...
    p(f"        送出：{'允許' if payload['submit_allowed'] else '阻擋'}（blockers {len(payload['blockers'])} 項）")
