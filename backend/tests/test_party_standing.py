...
#: 未遮罩的自然人實名。
#:
#: 上游 README 確實寫了「可以進 git」（ref 分支 `petition-procedure-review/README.md:84-86`），
#: 而**那句話沒有錯**——它回答的是「這是不是賽方資料集」：不是，是我方自爬的公開
#: 決定書，不受競賽資料隔離約束。錯在我們把它讀成了「這個檔案可以入庫」的完整結論。
#: **能不能進 git 至少有兩道閘：授權來源、以及內容裡有沒有個資。** 那句話只過第一道。
#: 這個教訓比「引用前要打開出處」更重要：出處我們打開了，但別人的授權聲明回答的是
#: 他被問的那個問題，不是我們要問的那個。
#:
