...
#: ⚠️ **「數字＋期間量詞」那條分支 2026-09-13 被拿掉了**（Ci 拍板）。
#:
#: 它原本是 `redirect` 判準的第二條：答案裡有數字又有天／日／月／年就攔。
#: 拿掉的依據是實際語料——QA 雲上實打的 27 則模型回答，人工標註其中 3 則真的下了
#: 期間結論：
#:
#:   只靠結論詞 ＋ `逾…期間` 樣式    抓到真的 3/3     誤抓 0/24
#:   額外加上「數字＋期間量詞」        多抓到真的 0     **多誤抓 5**
#:
#: 五則誤抓的實際觸發字串：
#:
#:   「處分日期是哪一天」    ← 疑問詞（「一」是中文數字、「天」是量詞）
#:   「在途期間 | 0 天」      ← **收文欄位的記載值**
#:   「環保署 98 年函釋」     ← 函釋字號裡的民國年
#:   「行政法院 92 年度判字」 ← 判解字號裡的民國年
#:
#: 第一則就是 `r10`——**這條修正本來就是為它寫的，而它自己過不了。**
#:
#: **為什麼不用特例把它們一一排除**：那五則分別是疑問詞、記載欄位值、字號裡的年份，
#: 三件互不相關的事。要靠特例排除就是三條彼此無關的例外規則堆在同一個判準上，
#: 而**那種判準沒有人有辦法推理它下一次會怎麼判**。下一個人看到漏網想補特例時，
#: 請先讀這一段。
#:
#: **殘餘風險是明確的，不要讀成「已驗證安全」**：「還有 5 天」這種純數字、不用任何
#: 結論詞的說法會漏。27 則裡找不到任何一則長那樣，但 **27 則不是 2700 則，樣本很小**。
#: 紅燈不受影響（它只看問題），所以最壞情況是「模型講了天數、紅燈亮了、答案照顯示」
#: ——比「正確答案被吃掉」溫和。
#:
#: **這條判準的天花板在這裡**：卷內**記載**的東西與模型**算出來**的東西，在文字上
#: 長得一樣。日期還有「年月日」的形狀可以認，**在途期間沒有，金額沒有**。
#: 所以「金額只紅燈不 redirect」（2026-09-13 走 (c)）不是特例，是同一件事的必然結果
#: ——redirect 的語意是「去看規則引擎算的」，而罰鍰與在途期間都沒有那個地方。


def answer_states_a_period_conclusion(answer: str) -> bool:
    """這則答案有沒有下期間結論（「已經逾期」「還在期間內」「逾法定期間」）。

    **`redirect` 附不附就看這一支，紅燈不看**（紅燈只看問題，維持保守）。
    兩個門檻不同是因為誤判的代價不對稱：紅燈誤判＝多一次人工覆核；
    **`redirect` 誤判＝承辦人拿不到答案**——前端拿到非 null 的 `redirect` 會把
    `answer` 整則丟掉、只顯示 reason 與 CTA（`frontend/src/store/app.js:563`，
    照契約 §2.4.1 一做的）。

    **不需要問題問過期限**：規則 1 只看問題，所以「這個案子怎麼樣？」→
    「距離期滿還有幾天」這種**模型自己講出來的**期間結論以前完全不受攔。
    §2.4.1 一的紅線是關於**答案內容**的，跟問題問了什麼無關。

    **只收結論詞與 `逾…期間` 樣式，不收裸數字**——理由與實測數據見上面那一大段。
    """
    text = answer or ""
...
#: 開了重排之後 `score` 是 cross-encoder 判的語意相關性，而法條查表的 1.0／0.0
#: 根本不是相似度，是條號在不在快照裡。
#:
