...
>
> **Ci 已拍板（2026-09-12）：內嵌在工作台①「案情與爭點」分頁。**
> 該分頁的摘要區欄位直接可改，下方放一個「我已核對這些欄位」的動作。
> **不另開一頁**——維持兩頁動線（US-1），且承辦人本來就要看那一區。

---

### US-10: 引用卡片要分開呈現「字號可驗」與「與本案相關」 [P1]

As a 承辦人,
I want 看得出一筆被引用的判解是「字號查得到」還是「真的跟我這件案子有關」,
So that 我不會把一筆隨機撈到的判決當成本案的法律依據。

**Acceptance Scenarios:**

- WHEN 某筆引用的燈號為綠（`citations[].state == "ok"`）, THEN 畫面顯示的文字
  **必須限定在「字號可對回法規快照」的意思**，不得寫成或看起來像「本系統確認這筆與本案相關」。
- WHEN 我點開一筆判解引用, THEN 看得到它的**檢索分數**與它是被哪一句查詢撈到的，
  讓我自己判斷相關性。
- WHEN 判解通道對本案沒有撈到任何東西, THEN 畫面明說
  **「本系統的判解庫（19 筆）未涵蓋本案爭點」**，而不是留白、也不是拿不相干的補位。

**Independent Test:**
對 `synthetic-ordinary-01`（程序逾期／寄存送達）跑一次，確認判解區塊要嘛是空的並附上面那句話，
要嘛列出的每一筆都帶分數且沒有被標成「已驗證＝可用」。

> **這條的來源是 F-1a，不是設計稿。** 現行守門驗的是「字號在不在白名單」，
> **不驗相關性**——19 筆判解全部在庫，所以任何一筆被引用都會拿到綠燈。
> 如果前端把那個綠燈呈現成「這筆依據沒問題」，就等於**把後端的一個已知盲區
> 放大成畫面上的保證**。畫面不能比系統知道得更多。
>
> **這是分層誠實在前端的直接延伸**：可驗算／有出處／請人工判斷這三層裡，
> 「字號查得到」屬於**有出處**，不是**可驗算**。相關性目前沒有任何一層保證得了，
> 它屬於**請人工判斷**。三者在畫面上必須分得出來。
...
- **FR-013**：引用的綠燈文案只能表示「字號可對回快照」，不得表示或暗示「與本案相關」（US-10）。
- **FR-014**：判解區塊為空時要說出原因（庫未涵蓋本案爭點），不得留白或以不相干結果補位（US-10）。
...
- **SC-007**：對 `synthetic-ordinary-01` 跑一次，判解區塊不出現任何未附檢索分數的「已驗證」標記；
  該區塊若為空，畫面要說出「判解庫未涵蓋本案爭點」而非留白。
...
### F-1：判解引用實測掛零 — 檢索策略問題，**但對本案不可解**

> **⏳ 引用本節任何分數前，先看這三條時效警告。**
>
> 1. **向量庫實際只有 2477 份文件**，不是 manifest 的 6997 筆。那 4520 筆環境部函釋
>    落在一個**沒被索引的 bucket**，對檢索零影響。（我一度誤信 KB 已擴充到 6997 並
>    據此標注本節過期——**那是錯的，`-72` 後來更正**。這行保留下來，是因為
>    「相信一則未經自己查證的更新」跟「從一個資料點推通則」是同一類錯誤。）
> 2. **12:20 與 12:32 之間索引變過**：同一句查詢、同一份文件、同一個深度，分數位移
>    0.016–0.082（最高行 94判1497：0.752 → 0.834）。成因未確認。
>    **任何橫跨這兩個時點的 A/B 對照都不可信**，包括下方 depth 15/50/100 那張表。
> 3. **引用深度數字時必須連查詢原文一起引**。三個 session 曾以為結論打架，
>    實際上是查的不是同一句。

> **⚠️ 本節的結論在 2026-09-12 被修正過一次，修正前的版本是錯的。**
> 初版標題寫「根因是檢索深度」，那是**從一個資料點推出的通則**：我只在「寄存送達」
> 那一題測過 `depth=15`，其餘題目的 `depth 15` 欄位是未測（表格裡標 `—`），
> 卻把結論寫成通則。`-72` session 獨立實測後推翻——**`depth=15` 在實質法題目上
> 本來就撈得到判解**（「行政罰法第7條第1項 故意過失」15 就有 2 筆，分數 0.787–0.789）。
> 保留這段紀錄是因為：**表格當時是誠實的，標題不是，而人只會讀標題。**
...
**定版結論（三個 session 各自實測後收斂，2026-09-12。本節結論當天被推翻兩次）：**

這題的答案要**三段拆開**，任何一段單獨拿出來都會誤導：

**（一）機制上，是檢索策略問題 —— `-77` 的論證成立，我原本沒想清楚。**
`rel.startswith()` 是 KB 回傳**之後**在本機篩；判解佔語料 0.8%，必然被 2347 筆決定書
洗出 top-N，**加深度救不了**（top-50 內判解仍 0 筆）。Bedrock KB 的 metadata `filter`
是在**檢索當下**限定範圍，同一句原本 0 筆的查詢立刻回 10 筆判解。
所以「庫裡沒有」作為**機制陳述**是不精確的。

**（二）但「判解庫沒有涵蓋本案爭點」作為事實陳述仍然成立，而且證據更強。**
不是撈不到，是**撈到的是雜訊**。我用刻意編造的不相干查詢做反例測試：

| 查詢 | 分數帶 | 說明 |
|---|---|---|
| 寄存送達 行政程序法第74條 送達生效日 不論實際領取（**本案原句**）| **0.611–0.634** | 回 6 份，**無一與寄存送達有關** |
| 行政罰法第7條第1項 故意或過失 責任條件（**判解庫真有覆蓋**）| **0.658–0.746** | top-2 精準命中行政罰法7條1項 |
| 餐廳廚房油煙異味 食品衛生稽查（**刻意編造，不相干**）| 0.613–0.642 | — |
| 犬隻未辦理寵物登記 未植入晶片（**刻意編造，不相干**）| 0.609–0.630 | 與本案原句**重疊 3/6 份** |

**本案原句的分數帶與兩個刻意編造的不相干查詢分不出來**；只有真有覆蓋的題目高一截。
`-77` 用另外兩句自編查詢（著作權、醫療）獨立重現，結論一致。

**（三）所以「可解」不成立。** filter 沒有把 0 變成 10，它把
**「找不到」變成「找到 6 筆不相干的」**。

---

#### ⛔ F-1a：由此衍生的風險，比原本的掛零嚴重 —— **可驗但不相干**

filter 回的那 6 筆會進 N5 的 `refs`，而 **`refs` 是模型可引用的白名單**。
於是模型可能在一份「寄存送達逾期不受理」的決定書裡引用
「臺北高等行政法院98年度訴字第2374號—非處分相對人」。

**而 N6 守門會放它過。** 守門檢查的是「字號在不在 laws-snapshot 白名單」——
那 19 筆判解全部在庫。**字號存在、引用可驗、燈號綠。**

**我們的守門驗字號存在，不驗相關性。**
0 筆是誠實的空白；6 筆不相干是一份**看起來有法院見解支撐、實際隨機抓**的決定書。

**單一分數門檻切不開**：我量到的縫約 0.02–0.05（落在 0.65 附近），
但 `-77` 那輪自編查詢上緣衝到 **0.655**，距離真相干下緣 **0.658 只差 0.003**。
**縫的寬度本身不穩定**，用一個常數切開的方案風險比想像中高。

**結論：`doc_kind` filter 先不要上**（三個 session 一致）。若要上，兩個前置條件：
1. 該通道要有**自己的相關性門檻**，且需 10–15 句涵蓋各案型的查詢校準
   （不能吃全域 `KB_MIN_SCORE=0.25`——**那個門檻在 19 份文件的範圍內從來沒擋掉任何東西**，
   不是「不是瓶頸」，是根本沒在守）；
2. **前端不得只顯示「已驗證」綠燈**（見 US-10）。

**修完之後的實測（2026-09-12，修改已完成，見下方 §Fix Applied）：**

| 查詢組 | 修後結果 |
|---|---|
| 實質法「行政罰法第7條第1項 故意過失」| 2 筆 / 2 份（最高行 109上780 `0.789`、108判531 `0.787`）|
| 實質法「訴願無實益 權利保護必要」| 5 筆 / 5 份（最高行 94判1497 `0.834`、**高高行** 109訴186 `0.787`、釋字546 `0.783`…）|
| **N5 對本案實際發出的四句原文** | **仍然 0 筆** ← 與修前相同，**這是預期行為，不是沒修好** |

**最後一列請特別注意**：`synthetic-ordinary-01` 跑起來時 N5 實際發出的四句查詢，
修完之後**仍然全部零命中**。這證實了語料覆蓋才是限制。
**不要為了讓這個 demo 案例「有東西」而去調低 `KB_MIN_SCORE`**——那是拿正確性換畫面。
看到這個 0 的人請不要再「修」一次。
...
- **~~Q4~~（已結案）**：n5 佔端到端 77%（39.5 秒）。**Ci 拍板：不動 n5，用動線掩護。**
  不拿草稿品質換 20 秒；改以 SSE 逐節點進度（`GET /api/runs/{id}/events`，後端已具備）
  讓等待有進展感，並在 demo 前先跑好一輪。反映在 US-1 的第三條 Acceptance Scenario。
...
## Fix Applied（2026-09-12，Ci 指派 `-76` 執行，未 commit）

**只動判解／函釋通道，沒有碰相似案通道**（`-72` 護欄 3；相似案的
`official 2 ＋ public_crawl 3` 是當天實跑驗證過的行為，決賽期間不動已驗證路徑）。

| 檔案 | 改了什麼 |
|---|---|
| `backend/retrieval/kb.py` | 新增 `REF_FETCH_DEPTH = 50`，取代 explicit-prefix 分支原本的 `top_k*3`＝15 |
| `backend/retrieval/kb.py` | `_retrieve()` 加 `dedupe_by_source` 參數，**預設 False**，只有判解通道開 |
| `backend/tests/test_live_plumbing.py` | 兩處寫死的 `15` 改成比對 `kb_module.REF_FETCH_DEPTH` 常數 |
| `backend/tests/test_live_plumbing.py` | 新增 3 條測試（深度不等式、判解通道去重、相似案通道刻意不去重）|

**去重的三個設計決定**（照 `-72` 的護欄 2）：
1. 按**來源檔路徑**去重，不是按分數或 chunk id——同一份判決的不同 chunk 分數本來就不同。
2. **在取 `top_k` 之前**去重，否則 5 個席次可能被同一份判決的 3 個 chunk 佔掉。
3. 保留**分數最高**的那個 chunk（KB 結果已按分數遞減，取第一次遇到的即可）。

**驗收證據：**
- `/opt/homebrew/bin/python3.14 backend/tests/run_all.py` → **266/266 全綠**（原 263 ＋ 新增 3）
- **變異測試**（確認新測試不是恆真）：
  - 把 `dedupe_by_source` 改回 `False` → `test_ref_channel_dedupes_chunks_of_the_same_document_before_taking_top_k` **FAIL** ✅
  - 把 `REF_FETCH_DEPTH` 改回 `15` → `test_ref_channel_fetch_depth_is_not_shallower_than_the_similar_case_channel` **FAIL** ✅
  - 還原後 266/266 全綠
- **實跑驗證**（真 KB，`min_score` 維持 0.25 未動）：見 §F-1 的修後結果表
