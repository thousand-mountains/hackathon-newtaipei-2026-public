...
### F-1：判解／函釋引用實測掛零，主因是**語料覆蓋**，不是檢索深度

> **⏳ 本節所有檢索數字量測於 2477 筆語料，已過期。**
> 2026-09-12 稍後 KB 擴充至 **6997 筆**（新增 4520 筆環境部函釋），
> 函釋佔比從 0.4% 升到 65%，**判解／函釋通道的命中排名一定會變**。
> 下列數字保留作為當時的紀錄與推理過程，**不要當成現況**。
> 索引完成後待重驗的三項：實質法查詢的去重後命中、相似案配額是否仍為
> `{official: 2, public_crawl: 3}`（最重要，分母變 2.8 倍）、N5 四句原句是否仍為 0。
...
**修正後的結論（兩個 session 交叉實測後的版本）：**

- **主因是語料覆蓋，不是深度。** `-72` 獨立實測證明 `depth=15` 在實質法題目上**本來就撈得到**
  判解（「行政罰法第7條第1項 故意過失」15 就有 2 筆、分數 0.787–0.789）。
  我那一題撈不到，是因為 19 筆判解對「寄存送達」這個題目幾乎沒有覆蓋。
- **`15 → 50` 仍值得改，但它是改善不是修 bug**：效益是「同一題從 3 筆變 6 筆」，
  不是「從 0 變有」。**不要把它當成「行政法院見解」這項需求被解決了。**
- **去重是真的必要**：兩個 session 獨立量到同一件事（`-72` 8→6 筆、我 5→2 份），
  同一份判決會以不同 chunk、不同分數重複出現，不去重會把 `top_k` 席次吃光。
- **硬限制（`-72` 逐類清點）**：判解就是只有 19 筆，**隊友的 S3 語料判解也是 19 筆、零增益**。
  所以「行政法院見解」在今天的條件下只有兩條路：**另外找語料來源，或誠實列進 roadmap**。
  這不是「還沒做」，是資料上的天花板——做簡報取捨時必須知道。
