...
| 帳號回收 | **已由 Ci 確認：環境會一直開到黑客松結束**（2026-09-12，Ci 口頭；非賽方書面，來源等級照實記）。先前兩說（開幕投影片寫 9/13 13:00 環境關閉／無一手來源的 9/15 08:00 推論）**皆不再作為規劃基準**。 | 不投資難拆的架構 |
...
- ~~已知限制：部署網址可能在 2026-09-13 13:00 失效~~ **已解除**：Ci 於 2026-09-12 確認環境會開到黑客松結束，網址在評審時段仍為活的。
- 先前把它列為交付風險（繳交要求部署網址、評審可能 13:00 後才點開）的理由已消失。**保留這段紀錄是因為當時的推理過程本身沒錯**——一手書面確實寫 9/13 13:00 關閉，而書面與口頭不一致時去問人是對的動作。
...
2. ~~**賽後網址是否需存活。**~~ **已結案**：Ci 2026-09-12 確認環境開到黑客松結束。賽後（頒獎之後）是否仍需存活未問，若日後需要再議。
