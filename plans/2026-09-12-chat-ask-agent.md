...
| AC5 | `RUN_MODE=bedrock`，先跑完一個 run 取 `$RID`，再 `curl -N -X POST .../chat -d '{"run_id":"'$RID'","message":"用 retrieve_refs 查『信賴保護原則』並引用編號"}'` | **先驗前置：至少一筆 `tool_result.hits[]` 非空**，否則判「本輪沒走檢索，AC5 未驗」——**不得判通過**。⚠️ 原本問「有沒有類似的訴願決定可以參考？」，2026-09-12 實測模型改用 `read_case` 而完全沒走檢索，`hits` 與 `refs` 同時為空，兩條斷言**空過**（空集合恆滿足）。問法改成指名工具。前置過了再驗：依序收到 `event: tool_call`、`event: tool_result`、一筆 `event: done`；`done.lamp` ∈ `{y,r}`；`done.refs[]` 的每個 `id` **要嘛**出現在前面某個 `tool_result.hits[].id`，**要嘛**是 `read_case` 註冊過的卷內 id（`L*`／`C*`，該筆 ref 的 `origin` 為 `record`）。卷內 id 不進 `hits`（spec §4.0 凍結 `read_case` 的 `hits` 恆為 `[]`），所以只比對 `hits` 會誤殺卷內引用 |
...
| AC11 | live 模式，把 `BEDROCK_MODEL_ID_DRAFT` 改成不存在的 id 後打 chat | 收到 `event: error` 且 **`grep -c 'event: done'` 為 0**。失敗不得偽裝成一則回答（spec §4.6）。**且 `stage` 必須是 `model`**——2026-09-12 實測它是 `tool`，因為 botocore 的 `ValidationException` 落到 `_stage_of()` 的兜底值；依 spec §4.6 的表前端會顯示「查詢來源失敗」但壞的是模型。兜底值已改成 `internal`（未知就說未知），botocore 例外判 `model` |
...
> **驗收自己會空過（2026-09-12 教訓）。** AC5 原本的問法讓模型走 `read_case` 而不碰檢索，
> `hits` 與 `refs` 同時為空，「refs 每個 id 都出現在 hits」這條在空集合上恆真——**它在「該驗的事
> 根本沒發生」時回報通過**。所以 AC5 現在有前置守衛：沒走檢索就判「未驗」，不判通過。
> 寫新驗收時先問一句：**這條在「什麼都沒發生」的情況下會不會綠？**
