...
# 「違反」**之前**的東西一律砍掉，不只砍開頭的「違反」兩字。
# 兩個理由，第二個是紅線：
#   1. 案型歸位：`受處分人○○違反空氣污染防制法` → `空氣污染防制法`
#   2. **個資**：第三方 corpus 的檔頭 `類別` 欄位裡夾著未遮罩的當事人姓名
#      （2026-09-12 清點：`受處分人吳平馨違反…`、`鍾明盛違反…` 等，
#      未遮罩的至少 2 筆）。側檔會進 KB、會顯示在相似案卡上——
#      那是把當事人姓名端到承辦人與評審面前（CONSTITUTION §6）。
CASE_TYPE_BEFORE_WEIFAN = re.compile(r"^.*?違反")
CASE_TYPE_SUFFIX = re.compile(r"事件$")

# 法規正名寫錯的，收斂到正確名稱。**只收「該法規不存在這個名字」的客觀錯字**，
# 不處理「多部法規合併」（`空氣污染防制法及廢棄物清理法`）或「非案型」
# （`追繳空氣污染防制費`）——那兩類是內容判斷，要 qa-legal 決定，不是正規化。
CASE_TYPE_TYPO = {
    "空氣污染管制法": "空氣污染防制法",     # 沒有「管制法」這部法
    "土讓及地下水污染整治法": "土壤及地下水污染整治法",   # 讓→壤
    "土壤及地下水污染整治法事": "土壤及地下水污染整治法",  # 「事件」被截成「事」
}
...
def canonical_case_type(s: str) -> str:
    """案型收斂成單一詞彙。順序有意義，不能對調。

    1. 異體字（汙→污）
    2. 砍掉「違反」**與它之前的一切**——同時處理案型歸位與姓名外洩
    3. 砍掉「事件」後綴
    4. 已知的法規正名錯字對照
    """
    raw = normalize_case_type(s.strip())
    out = CASE_TYPE_BEFORE_WEIFAN.sub("", raw)
    out = CASE_TYPE_SUFFIX.sub("", out).strip()
    out = CASE_TYPE_TYPO.get(out, out)
