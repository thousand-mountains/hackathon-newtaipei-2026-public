...
> | 我的第 4 版（現況） | **改變防線的位置**：送出封鎖改掛 case 層、完全不看字串；756 組交叉組合 0 穿過；等待第四輪獨立覆核 |
>
> 每一輪被打穿的共同教訓，第三輪覆核講得最直白：**只要最後一道防線是在比對字串，
> 就一定有盲點。** 我前三版都在把片語表做得更大，每一版都宣稱「這次窮舉了」，
> 每一版都被推翻。第 4 版不再修片語層去當防線，而是把防線換位置——
> **C 型案件一律不得送出，這條判準不看句子寫了什麼**，因此沒有任何寫法能繞過。
> 片語層退為提示層：它漏抓的後果變成「少標一個紅」，不是「放行一份系統沒看過的法律結論」。
>
...
新檔 `backend/tests/test_gate_hardening.py`（**49 條，全綠**），分四區，每區對應一輪覆核。
最新的整體量測（第 4 版）：
...
### AC4 全套測試與回歸 — ⚠️ 部分（118/119，唯一未過項為**加固前就存在**的既有失敗）
...
這輪最重要的改動不是任何一條規則，而是**把防線換了位置**。

前三版都在做同一件事：把「判斷這句話是不是主文」的字串規則做得更大更細，
每一版都宣稱這次是結構性的，每一版都被獨立覆核推翻（29/30 → 55/90 → 25/28）。
第四版承認那條路走不通：**片語層本質上是提示層，不可能是防線**。
送出封鎖改掛在一條不看文字的判準上——C 型案件一律不得送出——之後，
四輪覆核累積的所有繞法（756 組交叉組合）同時失效，因為它們全都在攻擊字串比對。

留給接手的人一句話：**如果有人要放寬 case 層封鎖（判斷卡 3 很可能會被要求放寬），
請同時知道你把防線交還給誰——交還給那個在四輪裡被打穿三輪的片語層。**
